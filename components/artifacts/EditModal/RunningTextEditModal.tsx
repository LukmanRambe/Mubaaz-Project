...
import { EditRunningTextFormValues } from '../../../ts/types/schema/RunningTextSchema';
import { generateUrutanOptions } from '../../../utils/generateData';
import { editRunningTextSchema } from '../../../utils/schema/runningTextSchema';
import ButtonLoading from '../Loading/ButtonLoading';

type RunningTextEditModalProps = {
...
const RunningTextEditModal = ({
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<EditRunningTextFormValues>({
    resolver: yupResolver(editRunningTextSchema),
  });
  ...
  const editRunningText = useMutation(
    'editRunningText',
    async () => {
      return fetchAxios.put(`/api/running-text/${idRunningText}`, {
        text: watch('text'),
    ...
    },
    {
      onSuccess: async (response) => {
        if (response.status === 200) {
          setShowToast(true);
          setToastMessage('Running Text berhasil diedit');
  ...
  const handleEditRunningText = () => {
    editRunningText.mutate();
  };

  useEffect(() => {
    if (runningText) {
      setValue('text', runningText?.text);
      setValue('urutan', {
        label: runningText?.urutan?.toString(),
        value: runningText?.urutan,
      });
    }
  }, [runningText]);
            ...
            <div className="relative flex flex-col max-w-md gap-5 px-3 py-5 mx-2 bg-white rounded-xl md:w-full md:h-auto md:p-6">
              ...
              {isFetchingRunningText ? (
                ...
                <form
                  ...
                  onSubmit={handleSubmit(handleEditRunningText)}
                >
                  ...
                  <div className="flex justify-end mt-10">
                    ...
                    >
                      {editRunningText.status === 'loading' ? (
                        <ButtonLoading />
                      ) : (
                        <>Edit</>
                      )}
...
export default RunningTextEditModal;
