...
import type { CreateKajianFormValues } from '../../../../ts/types/schema/KajianSchema';
import { createKajianSchema } from '../../../../utils/schema/kajianSchema';

const CreateKajian: NextPageWithLayout = () => {
  const {
    register,
    handleSubmit,
    control,
    formState: { errors },
  } = useForm<CreateKajianFormValues>({
    resolver: yupResolver(createKajianSchema),
  });
  ...
  const breadCrumbData = [
    ...
    {
      pageName: `Tambah Kajian`,
      pageHref: '',
    },
  ];

  const createKajian = useMutation(
    'createKajian',
    async () => {
      return fetchAxios.post('/api/kajians', requestBody);
    },
    {
      onSuccess: async (response) => {
        if (response.status === 200) {
          setShowToast(true);
          setToastMessage('Kajian berhasil ditambah');
      ...
      onError: async (error: AxiosError) => {
        if (error.response?.status === 401) {
          setToastMessage('Kajian gagal ditambah');
          if (toastMessage !== '') {
            setShowToast(true);
            setToastType('error');
          }
  ...
  const handleCreateKajian = async (data: CreateKajianFormValues) => {
    ...
    formData.append('waktu_awal', `${data.waktu_awal}:00`);
    formData.append('waktu_akhir', `${data.waktu_akhir}:00`);
  ...
  return (
    <>
      <Head>
        ...
        <meta name="Halaman Tambah Kajian" content="Tambah Kajian" />
      ...
      {isLoading ? (
        ...
        <div>
          ...
          <div>
            <HeaderTitle headerTitle="Tambah Kajian" />
            <BreadCrumb breadCrumbData={breadCrumbData} />
          </div>

          <div className="w-full p-5 mx-auto mt-10 bg-white lg:w-3/4 rounded-2xl">
            <form onSubmit={handleSubmit(handleCreateKajian)}>
              <div className="mb-5">
...
CreateKajian.getLayout = (page) => <DashboardLayout>{page}</DashboardLayout>;
