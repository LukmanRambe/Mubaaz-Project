...
const Ustadz: NextPageWithLayout = () => {
  ...
  const {
    ...
    refetch,
    isFetching,
  } = useRemoteGetAllUstadz(dataLimit, pageIndex, searchInput);
  const lastPage = useMemo(
    () => allUstadzData?.pagination?.lastPage,
    [allUstadzData?.pagination?.lastPage]
  );
  ...
  const tableHeads = useMemo(() => generateTableHeadsUstadz(), []);
  ...
  return (
    <>
      <Head>
        <title>List Ustadz</title>
        <meta name="Halaman List Ustadz" content="List Ustadz" />
      ...
      <div>
        <div className="flex flex-col gap-1">
          ...
          <PageHeader
            ...
            pageLink="/admin/ustadz/create"
            buttonText="Tambah Ustadz +"
                  ...
                  {isFetching ? (
                    ...
                    <>
                      {totalItems === 0 ? (
                        ...
                        allUstadzData?.data?.map((ustadz, index) => (
                          <tr
                            key={ustadz.id}
...
Ustadz.getLayout = (page) => <DashboardLayout>{page}</DashboardLayout>;
